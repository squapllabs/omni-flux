    ...
    if (data?.length > 1) {
        data?.forEach((itemdata: any, index: number) => {
            ...
            if (itemdata?.purchase_order_type === "Head Office") {
                ...
                headerData = [
                    ...
                    itemdata?.purchase_request_data?.requester_user_data?.first_name + " " + itemdata?.purchase_request_data?.requester_user_data?.last_name
                ]
            }
            else {
                headerData = [
                    ...
                    itemdata?.order_id,
                    format(new Date(itemdata?.order_date), 'MM/dd/yyyy'),
                    itemdata?.purchase_request_data?.selected_vendor_data?.vendor_name || "N/A",
                    itemdata?.indent_request_data?.total_cost,
                    itemdata?.indent_request_data?.requester_user_data?.first_name + " " + itemdata?.indent_request_data?.requester_user_data?.last_name

                ]
            }
            if (itemdata?.purchase_order_type === "Head Office") {
                itemsData = itemdata?.purchase_request_data?.purchase_request_quotation_details?.map((data: any, rowIndex: number) => {
                    return {
                        '': '',
                        ...
                        'Total Amount': data?.purchase_requested_quantity * data?.item_data?.rate
                    };
                })

            }
            else {
                itemsData = itemdata?.indent_request_data?.indent_request_details?.map((data: any, rowIndex: number) => {
                    return {
                        '': '',
                        'Sl No.': (rowIndex + 1).toString(),
                        ...
                        'Total Amount': data?.indent_requested_quantity * data?.bom_detail_data?.item_data?.rate
                    };
                })
            }
        ...
        if (data) {
            ...
            data?.map((item: any, rowIndex: number) => {
                // console.log("it", item);
                if (item?.purchase_order_type === "Head Office") {
                    itemsData = item?.purchase_request_data?.purchase_request_quotation_details?.map((data: any, rowIndex: number) => {
                    ...
                    itemsData = item?.indent_request_data?.indent_request_details?.map((data: any, rowIndex: number) => {
                        return {
                            '': '',
                            'Sl No.': (rowIndex + 1).toString(),
                            'Item Name': data?.bom_detail_data?.item_data?.item_name,

...
const AccountsProjectSummery = async (data: any) => {
    ...
    if (data?.length > 1) {
        data?.forEach((itemdata: any, index: number) => {
            ...
            headerData = [
                (index + 1),
                itemdata?.purchase_order_data?.purchase_request_data?.project_data?.code || 'N/A',
                itemdata?.purchase_order_data?.purchase_request_data?.project_data?.project_name || 'N/A',
                ...
                (itemdata?.requested_by_data?.first_name + " " + itemdata?.requested_by_data?.last_name) || 'N/A',
                itemdata?.paid_by_data === null ? "N/A" : (itemdata?.paid_by_data?.first_name + " " + itemdata?.paid_by_data?.last_name),
            ]
        ...
        data?.map?.((data: any, index: number) => {
            headerData = [
                (index + 1),
                data?.purchase_order_data?.purchase_request_data?.project_data?.code || 'N/A',
                data?.purchase_order_data?.purchase_request_data?.project_data?.project_name || 'N/A',
                ...
                data?.requested_by_data?.first_name + " " + data?.requested_by_data?.last_name || 'N/A',
                data?.paid_by || 'N/A',
            ]
        })

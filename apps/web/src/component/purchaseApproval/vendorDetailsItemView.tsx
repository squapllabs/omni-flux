...
const VendorDetailItemView = () => {
  ...
  const { data: getOneVendor } = getByQuoteVendorId(VendorId);
  ...
  return (
    <div>
      <div className={Styles.title}>
        <div>
          <h3>Item Details requested to Bharath Vendor</h3>
        </div>
        <div>
          <Button
            shape="rectangle"
            ...
            color="primary"
            icon={<BackArrowIcon />}
              onClick={() => navigate(`/vendor-select/${getOneVendor?.purchase_request_id}`)}
          >
            Back
          </Button>
        </div>
      ...
      <div className={Styles.tableContainer}>
          <div>
            <table>
              ...
              <tbody>
                {getOneVendor?.quotation_details?.map((data: any, index: number) => {
                  return (

...
const SiteExpensesView = (props: any) => {
  ...
  return (
    <div>
      <h3>{`Site Expense Information for ${
        props?.viewDocs?.expense_master_data?.master_data_name
          ? props?.viewDocs?.expense_master_data?.master_data_name
          : props?.viewDocs?.site_expense_name
      } Category`}</h3>
        ...
        <div className={Styles.content_container_left}>
          <div className={Styles.content_container}>
            <span>
              {' '}
              <b>Bill Number :</b>{' '}
              {props?.viewDocs?.bill_number
                ? props?.viewDocs?.bill_number
                : 'Not Provided'}
            </span>
            <span>
              <b>Description :</b>{' '}
              {props?.viewDocs?.description
                ? props?.viewDocs?.description
                : 'Not Provided'}
            </span>
          </div>
          <span>
            <b>Amount : </b>₹{' '}
            {props?.viewDocs?.total ? props?.viewDocs?.total : 'Not Provided'}
          </span>
        </div>
      </div>
      {documentURL ? (

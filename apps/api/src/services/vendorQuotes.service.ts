...
const createVendorQuotes = async (body: vendorQuotesBody) => {
    try {
        ...
        if (purchase_request_id) {
            const purchaseRequesterExist = await purchaseRequestDao.getById(purchase_request_id);
...
const updateVendorQuotes = async (body: vendorQuotesBody) => {
    try {
        ...
        let result = null;
        const vendorQuotesExist = await vendorQuotesDao.getById(
            vendor_quotes_id
        );
...
const getById = async (vendorQuotesId: number) => {
    try {
        let result = null;
        const vendorQuotesData = await vendorQuotesDao.getById(
            vendorQuotesId
        );
        if (vendorQuotesData) {
